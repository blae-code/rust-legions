...
Deno.serve(async (req) => {
  try {
    ...
    // A suspended or struck front accepts no orders — only state reads pass.
    if ((game.status === 'paused' || game.status === 'cancelled') && action !== 'getState') {
      return Response.json({
        error: game.status === 'paused'
          ? 'The Ministry has suspended this front — all orders are held until it resumes'
          : 'This front has been struck from the register',
      }, { status: 400 });
    }
