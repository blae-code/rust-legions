...
export default function InitiativeChip({ entry, index, seen, active, onPick }) {
  ...
  if (!seen) {
    return (
      <div
        className="w-[64px] shrink-0 border border-dashed border-border/70 bg-background/60 rounded-sm px-1 py-1 text-center opacity-70"
      ...
      >
        ...
        <p className="font-mono text-[9px] tracking-widest text-muted-foreground">UNKNOWN</p>
      </div>
    );
  }
  ...
  return (
    <button
      ...
      className={`w-[64px] shrink-0 border-t-2 ${edge} bg-card/85 rounded-sm px-1 py-1 text-center cq-metal
        ${active ? "ring-1 ring-brass-bright" : ""} ${spent ? "opacity-45" : ""}`}
    >
      <div className="flex items-center justify-between px-0.5">
        <span className="font-mono text-[10px] text-muted-foreground">{index + 1}</span>
        <span className="font-mono text-[10px] text-brass-bright">{score}</span>
      </div>
      {url ? (
        <Image src={url} alt="" className="cq-unit-portrait w-full h-9 rounded-sm" />
      ) : (
        <div className="w-full h-9 bg-secondary rounded-sm" />
      )}
      <p className="font-mono text-[9px] leading-tight truncate text-foreground/90 mt-0.5">
