...
> **AMENDMENT 2026-09-02 (Lane H, Amendment 2) — the closure of `docs/TECH_DESIGN.md` §7 Q5, and nothing else in that file.**
> The **WAVE 4 ADDENDUM** makes *"close `docs/TECH_DESIGN.md` §7 Q5"* a hard requirement of this lane and supplies the operator ruling that closes it (relic projects die with the keel; the captor loots unspent materials only). `docs/TECH_DESIGN.md` is assigned to **Lane G**, which merged in wave 1 — so the addendum asks Lane H to write in a file §3 does not give it.
> **The authority for the write is the addendum itself, and nothing else.** An earlier draft of this paragraph justified it with a session narrative — that the lane reported the conflict, and the orchestrator answered by re-issuing the instruction — which nothing in this repository records and which a later reader therefore could not check. That is the *dead-code-with-a-false-justification* shape the addendum warns about, in prose, inside the contract doc; it is removed. What stands on its own is the citation above: the WAVE 4 ADDENDUM is authoritative, it is dated after §3, and it names this closure as a hard requirement of this lane. This amendment is filed the way Lane F's Amendment 1 and Lane G's Amendment 3 filed theirs — the plan's own mechanism for a lane claiming a path §3 did not assign it — and it is filed **before** the edit it authorises, which is the mechanism's one requirement.
> **The claim is deliberately the narrowest that can carry the ruling: §7 question 5 and the one closing note beneath the §7 list.** Every other section of `docs/TECH_DESIGN.md` stays Lane G's, including §8's cost curve (parsed by `test/catalog-mirror.test.js`, untouched here), §9, §10 and §11. The question is renumbered nowhere: item 5 stays item 5, so every existing citation of *"§7 Q5"* still lands on it. Wave 4 has one lane in it, so no collision is possible; had Lane G been live, this amendment would have been a request to it instead.
> The same ruling is *also* encoded where this lane does own the surface — `docs/HERALD_VOICES.md` Shared Rule 7 and thirteen loss intercepts, and the Codex entry `works-lost-with-the-keel` — and `test/presets.test.js` gates all three, so the doc edit is the citation, not the only copy.
