...
2. `src/lib/*.js` — frontend mirrors used for display and pre-validation:
   ...
   - `src/lib/commandVehicles.js` — general command-vehicle chassis & refit-bay mod catalog
...
Built and live: territorial conquest, typed economy, buildings, 5 unit types, garrison combat, mass battles (generals, maneuvers, morale, signatures, veterancy, medals, **command vehicles with two-bay refit logistics**) with a post-battle combat-resolution screen, supply/logistics, weather, terrain/elevation modifiers, artillery bombardment, recon probes, army designs, faction point-buy + lifepath, NPC AI (3 doctrines), campaign mode, dispatch archive, war charts, patch-notes system, **diplomacy (Envoy Desk: truces/NAPs/trades, accords ledger, NPC dispositions)**, **mobile fortress-bases (vanilla slice: 3 module bays, refit yard, base movement, wreck-on-capture)**, **doctrine research tree + State Armory off-turn unlocks (`concurrentPlay`)**, Field Wire chat, Field Induction tutorial (`/walkthrough`), **The War Table** (`/star-map` — the canonical v2.x macro map per `docs/MACRO_MAP.md`: one orbital 3D campaign world picked at operation setup, node-and-route overlay, day-rate itineraries, radial node orders menus, great-circle march trails, tactical overlay; client-side sandbox — the flat `/macro-lab` is retired), **NPC Signals Intercepts** (background AI herald `npcHerald` — one in-character radio broadcast per NPC faction per turn, doctrine-voiced, stored as `NpcDispatch`), Asset Registry (`/asset-registry` — image plates + sound registry with generation prompts), rotating menu soundtrack + video backdrop reels.
