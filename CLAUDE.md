...
| `docs/MACRO_MAP.md` | **`[LOCKED v2]`** the macro map — The Ministry Chart: flat pannable war-chart, continents grown around node clusters, convoy lanes (hexes deleted 2026-07-18) |
...
Built and live: **the macro game on the Ministry Chart** (worlds of clustered continents + convoy lanes, day-rate column marches, dawn resolution, fog, settlement income, control victory, mass battles on the graph, supply envelope, land-only fortress-base movement — docs/MACRO_ENGINE.md M1–M3a), typed economy, mass battles (generals, maneuvers, morale, signatures, veterancy, medals, command vehicles), weather, faction point-buy + lifepath, NPC AI (3 doctrines), campaign mode, dispatch archive, war charts, patch-notes system, **diplomacy (Envoy Desk)**, **doctrine research tree + State Armory off-turn unlocks (`concurrentPlay`)**, Field Wire chat, Field Induction tutorial (`/walkthrough`), **The War Table** (`/star-map` planning sandbox), **the Cartography Bureau** (`/map-editor` — place settlements, the Ministry grows the world; published charts playable at setup), **NPC Signals Intercepts**, Asset Registry, rotating menu soundtrack + video backdrop reels. **The hex world model is deleted (2026-07-18).**
